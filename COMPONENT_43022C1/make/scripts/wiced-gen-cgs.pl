...
sub post_process_cgs
{
	...
	# move mpaf config to end after loading app data
    if($cgs_record->{file} =~ /43022C1/) {
        @lines = ();
...
sub post_app_cgs
{
    ...
    foreach my $cgs_record (@{$cgs_records}) {
        ...
        if($cgs_record->{type} ne 'patch' && defined $cgs_record->{tca_lines}) {
...
sub preview_sections
{
	...
	foreach my $section (@{$sections}) {
		next if !defined $section->{name};
		$section->{'direct_load'} = 1;
		$section->{direct_load} = 0 if $section->{name} eq '.pre_init_cfg';
...
sub output_cgs_elf
{
	...
	foreach my $section (@{$sorted_sections}) {
		...
		while($offset < length($section->{data})) {
			...
			$offset += $chunk;
			output_hdf_cfg_command($section->{name}, "$section->{name} from $file", $hdf->{$entry2code->{$name}},
									$data, 1, $direct_load && $section->{direct_load});
		}
	}
