...
else
{
    ...
    #Download FW
    if($direct_load == 1) {
         print "$chip_load -BLUETOOLMODE -PORT $com_port -LAUNCHADDRESS 0xFFFFFFFF -BAUDRATE $detected_baud -BTP $btp_file -NOERASE -CONFIG $config_file ${addl_flags}\n";
        qx{"$chip_load" -BLUETOOLMODE -PORT $com_port -LAUNCHADDRESS 0xFFFFFFFF -BAUDRATE $detected_baud -BTP "${btp_file}" -NOERASE -CONFIG ${config_file} -LOGTO "$download_log_path" ${addl_flags}};
    }
    else { # 43022C1 is currently using RAM-based config without minidriver.
        print "$chip_load -BLUETOOLMODE -PORT $com_port -BAUDRATE $detected_baud -NOERASE -NOVERIFY -BTP $btp_file -CONFIG $config_file ${addl_flags}\n";
        qx{"$chip_load" -BLUETOOLMODE -PORT $com_port -BAUDRATE $detected_baud -NOERASE -NOVERIFY -BTP "${btp_file}" -CONFIG ${config_file} -LOGTO "$download_log_path" ${addl_flags}};
    }
	exit ($? >> 8);
}
